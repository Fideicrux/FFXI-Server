...
-- Structure:            [spellId] = { Tier, Main_Effect, Spell_Level, Base_Power, Base_Duration, Composure, Always_Overwrite, Tick_Seconds },
    ...
    -- Regen
    [xi.magic.spell.REGEN        ] = { 1, xi.effect.REGEN,         21,    5,   90, true,  false, 0 },
    [xi.magic.spell.REGEN_II     ] = { 2, xi.effect.REGEN,         44,   12,   90, true,  false, 0 },
    [xi.magic.spell.REGEN_III    ] = { 3, xi.effect.REGEN,         66,   20,   90, true,  false, 0 },
    [xi.magic.spell.REGEN_IV     ] = { 4, xi.effect.REGEN,         86,   30,   90, true,  false, 0 },
    [xi.magic.spell.REGEN_V      ] = { 5, xi.effect.REGEN,         99,   45,   90, true,  false, 0 },
...
xi.spells.enhancing.calculateEnhancingFinalPower = function(caster, target, spell, basePower, spellGroup, tier, spellEffect)
    ...
    if caster:hasStatusEffect(xi.effect.STYMIE) and basePower > 0 then
        ...
        local stymieMultipliers = {
            [xi.effect.PROTECT] = 1.5,  -- 50% increase (doubled potency reduced to 1.5x base)
            [xi.effect.SHELL]  = 1.1,  -- 10% increase (doubled potency reduced to 1.1x base)
        }
        ...
        -- Determine multiplier: Use table for specific effects, else check Enspells, else default
        local multiplier = stymieMultipliers[spellEffect] or (isEnspell and 1.5) or 2.0
...
xi.spells.enhancing.calculateEnhancingDuration = function(caster, target, spell, spellId, spellGroup, spellEffect)
    ...
    if
        useComposure and
        caster:hasStatusEffect(xi.effect.COMPOSURE)
    then
        duration = duration * 3
    end
...
xi.spells.enhancing.useEnhancingSpell = function(caster, target, spell)
    ...
    -- Refresh
    elseif spellEffect == xi.effect.REFRESH then  --Remove after testing.
        -- if
        --     tier < 3 and
