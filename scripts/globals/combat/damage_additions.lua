...
xi.combat.damage.souleaterAddition = function(actor)
    ...
    local bonusDamage            = math.floor(actor:getHP() * (0.2 + souleaterEffect + souleaterEffectII))

    if bonusDamage > 0 then
        local selfDamage = (bonusDamage * stalwartSoulMultiplier) / 4
...
xi.combat.damage.consumeManaAddition = function(actor)
    ...
    local bonusDamage = math.floor(actor:getMP() / 2)
    actor:delMP(bonusDamage)
